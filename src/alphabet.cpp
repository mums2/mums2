...
#include "RdisopHeaderFiles/compose_f_gx_t.h"
#include "RdisopHeaderFiles/compose_f_gx_hy_t.h"
#include "RdisopHeaderFiles/alphabettextparser.h"
...
bool Alphabet::hasName(const name_type& name) const {
        return std::find_if(elements.begin(), elements.end(), [name](const element_type& elem) {
...
void Alphabet::sortByNames() {
        std::sort(elements.begin(), elements.end(), [](const element_type& lhs, const element_type& rhs) {
