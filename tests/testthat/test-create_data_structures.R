...
test_that("generate_a_combined_table returns a data.frame with proper data",
{
  ...
  matched_data_and_cluster_data <- generate_a_combined_table(matched_data = ms2_data, 
                                                             cluster_data = cluster_results)
  expect_true(ncol(matched_data_and_cluster_data) == 23)
  expect_true(all(ms2_data$samples %in% colnames(matched_data_and_cluster_data)))
  ...
  with_rt_in_seconds_column <-  generate_a_combined_table(matched_data = ms2_data)
  expect_true("RTINSECONDS" %in% colnames(with_rt_in_seconds_column))

  data <- change_rt_to_seconds_or_minutes(data, "minutes")
  ms2_data <- ms2_ms1_compare(mgf_files, data, 2, 100)
  with_rt_in_minutes_column <-  generate_a_combined_table(matched_data = ms2_data)
...
data <- import_all_data(peak_table = test_path("exttestdata", "peak_table.csv"), 
  ...
  mgf_files <- test_path("exttestdata", "12152023_Coculture_with_new_JC1.gnps.mgf")
  ms2_data <- ms2_ms1_compare(mgf_files, data, 2, 6)
  expect_error(generate_a_combined_table(matched_data = mgf_files), "matched_data must")
  expect_error(generate_a_combined_table(matched_data = ms2_data, annotations = mgf_files), "annotations must")
  expect_error(generate_a_combined_table(matched_data = ms2_data, cluster_data = mgf_files), "cluster_data must")
