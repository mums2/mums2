...
data <- import_all_data(
  peak_table = mums2::mums2_example("boryillus_peaktable.csv"), 
  metadata = mums2::mums2_example("boryillus_metadata.csv"), 
  format = "Progenesis")
```
...
read.csv(mums2::mums2_example("boryillus_peaktable.csv"), check.names = FALSE)
```
...
read.csv(mums2::mums2_example("boryillus_metadata.csv"), check.names = FALSE)
```
...
mpactr::get_peak_table(filtered_data)$rt
```
...
that would take a very long time to run. In our experience such a large value does not yield meaningfully more precision than using a smaller value. In this case, we would encourage one to use a value of 4000 as is illustrated in the rest of this tutorial.
...
```{r  alpha_summary, R.options=list(max.print=10)}
alpha <- alpha_summary(
  community_object = community_w_omus, size = 4000, threshold = 100,
...
as_tibble(pcoa$points, rownames = "sample") |>
  inner_join(mpactr::get_meta_data(filtered_data), by = c("sample" = "Injection")) |>
...
```{r samples}
  ...
  mpactr::get_meta_data(filtered_data)[, c("Injection", "Sample_Code")]
```
...
metadata <- mpactr::get_meta_data(filtered_data)
alpha$group <- sapply(alpha$name, function(x) {
   metadata$Biological_Group[[which(x == metadata$Sample_Code)[[1]]]]
}, USE.NAMES = FALSE)
