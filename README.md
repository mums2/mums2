...
Once we have preprocessed all of our data (filtered and added a ms2
spectra), we can annotate ms2 spectra given a reference database. Using
supporting functions for `tidymass` we are able to created a simplistic
...
score spectra, GNPS (Global Naturee Products Social Molecular
...
Paper](10.1016/1044-0305(94)87009-8). In order to ensure a user could
run through our pipeline in a resonable amount of time, we modified the
algorithm to make it much more efficent than that of gnps. We have
confirmed our results to be identical as it still uses the same
...
similarity between spectras (and more to be found
[here](https://www.nature.com/articles/s41592-021-01331-z)), but the
results are comparable to that of GNPS’ method.
...
opticlust](https://journals.asm.org/doi/10.1128/mspheredirect.00073-17)).
This allows us to cluster our data into Operational Metageonomic Units
(OMU) which are need to conduct diversity calculations on the data. In
order to cluster the data, we have implemted a wrapper over `clusturs`
function, `cluster_data()`. Using this function you are able to
determine how you want to cluster your data and it returns a shared
dataframe of all of your data (Using this shared data frame you have the
option to save it as a csv and using other software like `mothur` for
...
data, the created object may require a substantial amount of data.

With your community object we can began to calculate diversity metrics.
We have implemented a function call `diversity()`. This function will
conduct a diversity index calculation based on the index you pass to it.
You can conduct alpha diversity (simpson, shannon) or beta diversity
(bray curtis). We also have implemented a function call `rarefaction()`
for rarefaction calculations of the data. This will allow users to
conduct more efficent calculations. Finally, we have created the
`averaged_subsampled_dissimilarity()` function. This function will run a
diversity index and rarefaction “n” number of times, then it will
calculate the average of the all the runs. This allows users to ensure
all of there data was accounted for when attempting to analysis the
diversity of the data.

Thus we believe this mums2 will benefit those attempting to analysis
untargeted metabolites inside of their data and other calculations
related to metabolomic analysis. Whether you want to run this up to step
3 or through the whole pipeline we belive this package will encourage
scientist to easily analysis there data. This package is also open
source and free to use, which means scientist are able to create
reproudicible analysis that others can copy. They also have the ability
to create pull requests or ask for the creation of new features and
methods that can be incorpated inside of mums2. This not only opens the
door for a rich community of scientist in this area of research to join
together, but will allow advance the community of researchers as a
whole.
